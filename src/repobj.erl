...
inherit(OldPid, PrtclMod, PrtclArgs, Nodes, Timeout) ->
   ...
   Members = case PrtclMod:type() of
      ?REPL ->
         [ replica:new(PrtclMod, PrtclArgs, no_sm, N) || N <- Nodes ];
      ?PART ->
         [ {T, replica:new(PrtclMod, PrtclArgs, no_sm, N)} || {T,N} <- Nodes ]
   end,
   ...
   ShardAgent = case (PrtclMod:type() == ?PART) and (OldConfType /= ?SINGLE) of
      true -> OldPid;
      false -> ?NoSA
   end,
